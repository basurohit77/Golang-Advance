...
// Test Incident for IBM Cloud, not Gaas
func TestIncident(database *sql.DB) int {
	FCT := "TestIncident "
	failed := 0
	...
	queryStr5 := db.RESOURCE_QUERY_CRN + "=crn:v1:aa:dedicated:service-NAME2:::::" + "&" + db.INCIDENT_QUERY_CREATION_TIME_START + "=" + creationTimeStart5 + "&" + db.INCIDENT_QUERY_CREATION_TIME_END + "=" + creationTimeEnd5
	...
	log.Println("Test get Incident")
	queryStr6 := db.RESOURCE_QUERY_CNAME + "=bluemix&" + db.RESOURCE_QUERY_CNAME + "=public&" + db.RESOURCE_QUERY_SERVICE_NAME + "=service-name1&" + db.INCIDENT_QUERY_CREATION_TIME_START + "=" + creationTimeStart5 + "&" + db.INCIDENT_QUERY_CREATION_TIME_END + "=" + creationTimeEnd5
	...
	log.Println("======== " + FCT + " Test 6a GetIncidentByQuery cname & ctype & service-name & creation_time_start & creation_time_end ========")

	log.Println("Test get Incident")
	queryStr6a := db.RESOURCE_QUERY_CNAME + "=bluemix&" + db.RESOURCE_QUERY_CNAME + "=public&" + db.RESOURCE_QUERY_SERVICE_NAME + "=service-name1&" + db.INCIDENT_QUERY_CREATION_TIME_START + "=" + "2018-07-07T22:00:01Z" + "&" + db.INCIDENT_QUERY_CREATION_TIME_END + "=" + "2018-07-07T22:02:01Z"
	log.Println("query: ", queryStr6a)
	incidentReturn6a, total_count6a, err6a, _ := db.GetIncidentByQuery(database, queryStr6a, 0, 0)
	log.Println("total_count6a: ", total_count6a)
	log.Println("incidentReturn6a: ", incidentReturn6a)

	if err6a != nil {
		log.Println("Get Incident failed: ", err6a.Error())
		log.Println("Test 6a: Get Incident FAILED")
		failed++
	} else if total_count6a != 1 {
		log.Printf("Error: total_count: %d, expecting: 1\n", total_count6a)
		log.Println("Test 6a: Get Incident FAILED")
		failed++
	} else if len(*incidentReturn6a) != 1 {
		log.Printf("Error: Number of returned: %d, expecting: 1\n", len(*incidentReturn6a))
		log.Println("Test 6a: Get Incident FAILED")
		failed++
	} else {
		log.Println("Test 6a: Get Incident PASSED")
	}
	...
	resource9a := datastore.ResourceInsert{
		CRNFull:           "crn:v1:bluemix:public:service-name9a:location9a:scope9a:service-instance9a:resource-type9a:resource9a",
		State:             "ok",
		OperationalStatus: "none",
		SourceID:          "resource_id9a",
		Source:            "test_source9a",
		Status:            "ok",
		RegulatoryDomain:  "regulatory domain 1",
	...
	resource9b := datastore.ResourceInsert{
		CRNFull:           "crn:v1:aa:public:service-name9b:location9b:scope9b:service-instance9b:resource-type9b:resource9b",
		State:             "ok",
		OperationalStatus: "none",
		SourceID:          "resource_id9b",
		Source:            "test_source9b",
		Status:            "ok",
	...
	incidentCRN9a := []string{"crn:v1:bluemix:public:service-name9a:location9a::::"}
	incidentCRN9b := []string{"crn:v1:bluemix:public:service-name9a:location9a::::", "crn:v1:aa:public:service-Name9b:location9b:scopeX:::"}
	...
	incidentJuncGet9, err9A, _ := db.GetIncidentJunctionByIncidentID(database, record_idinc9)
	if err9A != nil {
		log.Println("Failed getting IncidentJunction")
		failed++
	}
	if len(*incidentJuncGet9) != 1 {
		log.Println("Failed: Should have 1 entries in IncidentJunction, len(*incidentJuncGet)=", len(*incidentJuncGet))
		failed++
	} else {
		log.Println("First Update Incident passed")
	}

	// modify incident9 data
	incidentCRN9c := []string{"crn:v1:bluemix:public:service-name9a:location9a::::", "crn:v1:aa:public:service-Name9b:location9b::::"}
	incident9.CRNFull = incidentCRN9c

	// update incident with added crn
	err, _ = db.UpdateIncident(database, &incident9)
	if err != nil {
		log.Println("Failed in UpdateIncident")
		failed++
	...
	queryStr10a := db.RESOURCE_QUERY_CRN + "=crn:v1:bluemix:public:service-name2:::::" + "&" + db.INCIDENT_QUERY_CREATION_TIME_START + "=" + creationTimeStart10 + "&" + db.INCIDENT_QUERY_CREATION_TIME_END + "=" + creationTimeEnd10
	...
	queryStr10b := db.RESOURCE_QUERY_CRN + "=crn:v1:bluemix:public:service-name2:::::" + "&" + db.INCIDENT_QUERY_CREATION_TIME_START + "=" + creationTimeStart10 + "&" + db.INCIDENT_QUERY_CREATION_TIME_END + "=" + creationTimeEnd10 + "&" + db.INCIDENT_COLUMN_PNP_REMOVED + "=true"
	...
	queryStr10c := db.RESOURCE_QUERY_CRN + "=crn:v1:bluemix:public:service-name2:::::" + "&" + db.INCIDENT_QUERY_CREATION_TIME_START + "=" + creationTimeStart10 + "&" + db.INCIDENT_QUERY_CREATION_TIME_END + "=" + creationTimeEnd10
	...
	queryStr10d := db.INCIDENT_COLUMN_PNP_REMOVED + "=false"
	...
	queryStr10e := db.INCIDENT_COLUMN_PNP_REMOVED + "=true,false"
	...
	if len(*incidentReturn10e) != 3 {
		log.Printf("Error: Number of returned: %d, expecting: 3\n", len(*incidentReturn10e))
		log.Println("Test 10e: Get Incident FAILED")
		failed++
	} else {
