...
func NewIAMAuth(svcName string) *IAMAuth {
	...
	iamauth := &IAMAuth{
		...
		iamURL: (func() string {
			if envIAMURL := secret.Get("IAM_URL"); envIAMURL != "" {
				return envIAMURL
			}
	...
	if flag := secret.Get("IAM_BYPASS_FLAG"); flag == "true" {
...
func (iam *IAMAuth) configPep() {
	...
	var apiKey = secret.Get("CLOUD_SERVICE_API_KEY")
	if apiKey == "" {
...
func (iam *IAMAuth) checkServiceAPIKey() error { // TODO: will be removed in the future
	if iam.serviceAPIKey == "" {
		if svcAPIKey := secret.Get("CLOUD_SERVICE_API_KEY"); svcAPIKey != "" {
