...
// Logger is an interface describing the logging contract that must be met in order for log events to propagate
...
func (l Level) String() string {
	return [...]string{"LevelNotSet", "LevelDebug", "LevelInfo", "LevelError"}[l]
}
...
// BasicLogger describes the logger object wrapping the standard log library
type BasicLogger struct {
	LogLevel Level
	log      *log.Logger
}

// NewDefaultBasicLogger creates a BasicLogger with an error log level and the output set to os.Stdout
func NewDefaultBasicLogger() (*BasicLogger, error) {
	return NewBasicLogger(LevelError, os.Stdout)
}

// NewBasicLogger creates a BasicLogger using the provided level and output `io.Writer` and returns it to the caller
func NewBasicLogger(level Level, output io.Writer) (*BasicLogger, error) {
...
func (logger *BasicLogger) Debug(v ...interface{}) {
...
func (logger *BasicLogger) Info(v ...interface{}) {
...
func (logger *BasicLogger) Error(v ...interface{}) {
	if logger.LogLevel <= LevelError {
		logger.logLn(LevelError, v...)
	}
}

func (logger *BasicLogger) logLn(level Level, v ...interface{}) {
	switch level {
	case LevelDebug:
